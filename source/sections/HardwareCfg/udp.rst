...
.. field-list-table:: Leandc UDP node
   ...
   * :attr:    :xmlref:`LocalIPaddr`
     :val:     0.0.0.0 ... 255.255.255.254
     :desc:    Local IPv4 address. There must be a local Ethernet interface with this address assigned in order for leandc to open the socket. UDP messages will be received if remote peer sends them to Ethernet interface with this local address. Address 0.0.0.0 can be used to receive UDP messages through any running Ethernet interface. (default address 0.0.0.0) :inlinetip:`Attribute is optional and doesn't have to be included in configuration, default value will be used if omitted.`
