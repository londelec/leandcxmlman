...
It is possible to adjust incoming and outgoing message timestamps in case if LEANDC unit communicates to 
...
unit operating system directory:
...
If required the System default time zone can be changed as described in Leandc_SSH manual section '*Change 
Leandc operating system time zone*'. The System default time zone is used for maintaining internal time of the 
LEANDC unit. Internal time is used for time synchronization, time-stamping logfile records, creating logfile 
names, etc.